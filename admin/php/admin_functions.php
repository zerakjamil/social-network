...
function verifyUser($user_id){
    global $db;
    $sql = "UPDATE users SET counter = 2 WHERE id = $user_id";
    mysqli_query($db,$sql);
    $query="UPDATE users SET verify=1 WHERE id ='$user_id'";
...
$toggle = isset($_POST['locked']) && $_POST['locked'] == 1 ? 1 : 0;
    $query="UPDATE admin SET full_name='$full_name',email='$email',password='$password',password_text='$password_text',m_mode='$toggle' WHERE id=$user_id";
    return mysqli_query($db,$query);
}
?>